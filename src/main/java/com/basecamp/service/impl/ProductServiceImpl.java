...
public class ProductServiceImpl implements ProductService {
    ...
    public GetRaceStatusResponse homework() {
        ArrayList<String> handledTasks = new ArrayList<>();

        for(int i = 0; i < 5; i++){
            try {
                handledTasks.add(taskService.handleBugByExecutor().get());
        ...
        //Console
        handledTasks.forEach(position -> log.info(position));
        log.info("\n");

        //Response
        String status = "The race is still going on";
